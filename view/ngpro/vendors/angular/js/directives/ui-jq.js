                    ...
                    function getOptions(){
                        ...
                        if (attrs.uiOptions) {
                            ...
                            if(tAttrs.uiJq == 'dataTable'){
                                ...
                                var columnDefs = attr.hasOwnProperty('columnDefs')?attr.columnDefs:[];
                                var setColumnDefs = function (columnDefs,type){
                                    columnDefs.push({
                                        targets: parseInt(i),
                                        "mRender": function (data) {
                                            type = type.replace("{data}", data);
                                            type = type.replace("{id}", data);
                                            return type;
                                        },
                                ...
                                for(var i in columns){
                                    var type = columns[i].hasOwnProperty('type')?columns[i].type:'';
                                    if(type){
                                        columnDefs = setColumnDefs(columnDefs,type);
                                    }
                                }
                                //end
                                ...
                                if(!is_oper || (attr.c_operate == true)){
                                    columnDefs.push({
                                        targets: -1,
                                        "mRender": function (data) {
                                            return '<a ng-click="edit(\''+attr.model+'\','+data.id+')" class="active"><i class="fa fa-edit"></i></a>'+
                                                '&nbsp;<a ng-click="del(\''+attr.model+'\','+data.id+')" class="active"><i class="fa  fa-trash-o"></i></a>';
                                        },
                                        "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
                                            $compile(nTd)(scope);
                                        },
                                ...
                                /*默认参数设置*/
                                attr.columnDefs = columnDefs;  //把设置好的列赋值给列对象
                                if(!attr.hasOwnProperty('serverSide')) attr.serverSide = true; //设置服务器模式
                                if(!attr.hasOwnProperty('ordering'))  attr.ordering = false;  //设置排序
                                attr.searching = true;  //禁用默认搜索
                                //end
